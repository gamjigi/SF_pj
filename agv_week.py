...
def handle_client(client_socket):
    while True:
        data = client_socket.recv(1024).decode().strip()  # 데이터를 수신하고 공백을 제거합니다.
        rospy.loginfo(data)
            ...
            try:
                ...
                if goal_x is not None and goal_y is not None:
                    if move_to_goal(goal_x, goal_y):
                        rospy.loginfo(f"{data} 지점에 도착했습니다. 다음 좌표를 기다립니다.")
                    else:
                        rospy.loginfo(f"{data} 지점 도달 실패. 다시 시도하세요.")
                else:
                    rospy.loginfo("유효하지 않은 좌표입니다. 다시 시도하세요.")
            except ValueError:
                rospy.loginfo("잘못된 입력입니다. 숫자를 입력하세요.")
        else:
            rospy.loginfo("유효하지 않은 데이터입니다. 다시 시도하세요.")
            move_to_goal(-0.3,-0.5)
            break  # 유효하지 않은 데이터가 오면 다시 대기합니다.
